...
agent to confirm behavior against the *running* application —
`project_eval` for code paths, `execute_sql_query` for data shape —
instead of inferring it from source. When it isn't, the packet says
so, and verification falls back to the test suite.

## Why this is the payoff

The ladder's lower levels each strengthen a guarantee; packets are
where the guarantees compound. Because the map
...
which is the point of having the rest.
